...
INSERT INTO CLIENTES VALUES(TIPO_CLIENTE (2, 1149312311, 'Carlos Andres',3132567840,'C','M','Piel sensible'));
...
INSERT INTO TATOOARTISTS VALUES(TIPO_TATOOARTIST (1,
 ...
 'PELAO',
 'PELAO@GMAIL.COM',
  (SELECT REF(C) FROM CONTRACTS C WHERE C.idContract=1),
 'C://TATUADORES//PELAO.PNG',
 'CAMBIAR POR LA LISTA DE TRABAJOS'));

--CONSULTA
SELECT VALUE(T) FROM TATOOARTISTS T;
...
INSERT INTO PAYMENTS VALUES(TYPE_PAYMENT (2, TO_TIMESTAMP('2017/06/05 10:13:18', 'YYYY/MM/DD HH:MI:SS'), 60000));
...
--================TRABAJO========
--LISTA DE PAGOS
CREATE OR REPLACE TYPE PAYMENTS_LIST IS TABLE OF REF TYPE_PAYMENT;
--LISTA DE CITAS
CREATE OR REPLACE TYPE APPOINMENTS_LIST IS TABLE OF REF TYPE_APPOINMENT;
...
(
       ...
       TATUADOR        REF     TIPO_TATOOARTIST,
       TATUAJE         REF     TYPE_TATOO,
       COSTO_TOTAL             DECIMAL,
       MEMBER FUNCTION PAGO RETURN DECIMAL, --Lo que se ha pagado
       MEMBER FUNCTION DEUDA RETURN DECIMAL  --la deuda del trabajo
);
DROP TYPE TYPE_TATOOWORK;
DROP TABLE TATOOWORKS;
--CUERPO DE TATOOWORK
CREATE OR REPLACE TYPE BODY TYPE_TATOOWORK AS
  MEMBER FUNCTION PAGO RETURN DECIMAL IS
  pagados decimal;
  BEGIN
    pagados:=0;
    for i in PAGOS.first .. pagos.last loop
      pagados:=pagados+pagos(i).pago;
    end loop;
    RETURN pagados;
  END;
  
  MEMBER FUNCTION DEUDA RETURN DECIMAL IS 
  BEGIN
    RETURN COSTO_TOTAL-PAGO();
  END;
END;
...
(SELECT REF(C) FROM CLIENTES C WHERE C.CODIGO=1),
PAYMENTS_LIST((SELECT REF(P) FROM PAYMENTS P WHERE P.CODIGO=1)),
APPOINMENTS_LIST((SELECT REF(A) FROM APPOINMENTS A WHERE A.CODIGO=1)),
(SELECT REF(T) FROM TATOOARTISTS T WHERE T.CODIGO=1),
(SELECT REF(F) FROM TATOOS F WHERE F.CODIGO=1),
200000
));
INSERT INTO TATOOWORKS VALUES(TYPE_TATOOWORK (2,
(SELECT REF(C) FROM CLIENTES C WHERE C.CODIGO=1),
PAYMENTS_LIST((SELECT REF(P) FROM PAYMENTS P WHERE P.CODIGO=1)),
APPOINMENTS_LIST((SELECT REF(A) FROM APPOINMENTS A WHERE A.CODIGO=1)),
(SELECT REF(T) FROM TATOOARTISTS T WHERE T.CODIGO=1),
(SELECT REF(F) FROM TATOOS F WHERE F.CODIGO=2),
200000
));
INSERT INTO TATOOWORKS VALUES(TYPE_TATOOWORK (3,
(SELECT REF(C) FROM CLIENTES C WHERE C.CODIGO=2),
PAYMENTS_LIST((SELECT REF(P) FROM PAYMENTS P WHERE P.CODIGO=1),
              (SELECT REF(P) FROM PAYMENTS P WHERE P.CODIGO=2)),
APPOINMENTS_LIST((SELECT REF(A) FROM APPOINMENTS A WHERE A.CODIGO=1)),
(SELECT REF(T) FROM TATOOARTISTS T WHERE T.CODIGO=1),
(SELECT REF(F) FROM TATOOS F WHERE F.CODIGO=2),
220000
...
--CONSULTA
SELECT t.codigo, T.COSTO_TOTAL,T.PAGO(),T.DEUDA() FROM TATOOWORKS T;
SELECT * FROM TATOOWORKS;


--BLOQUE ANONIMO PARA APLICAR PAGO
DECLARE
  mi_WORK TYPE_TATOOWORK;
BEGIN
  SELECT VALUE(t) INTO mi_WORK FROM TATOOWORKS t WHERE T.CODIGO=1;
  mi_WORK.APLICAR_PAGO(90000);
END;
