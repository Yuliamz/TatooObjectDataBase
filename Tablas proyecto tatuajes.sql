...
(
       ...
       MEMBER PROCEDURE APLICAR_PAGO(VALOR DECIMAL)
);
--CUERPO DE TATOOWORK
CREATE OR REPLACE TYPE BODY TYPE_TATOOWORK AS
  ...
  MEMBER PROCEDURE APLICAR_PAGO(VALOR DECIMAL) IS
  DEVOLUCION DECIMAL;
  BEGIN
    DEVOLUCION:=0;
    IF VALOR<=DEUDA()
      THEN 
      PAGOS.EXTEND();
      PAGOS(PAGOS.LAST).CODIGO := PAGOS.LAST;
      PAGOS(PAGOS.LAST).FECHA := SYSTIMESTAMP;
      PAGOS(PAGOS.LAST).PAGO := VALOR;
      DBMS_OUTPUT.PUT_LINE('EL CLIENTE PAGO: '||VALOR||' Y SU NUEVA DEUDA ES: '||DEUDA());      
      ELSE 
      DEVOLUCION:=VALOR-DEUDA();
      PAGOS.EXTEND();
      PAGOS(PAGOS.LAST).CODIGO := PAGOS.LAST;
      PAGOS(PAGOS.LAST).FECHA := SYSTIMESTAMP;
      PAGOS(PAGOS.LAST).PAGO := DEUDA();
      DBMS_OUTPUT.PUT_LINE('EL CLIENTE PAGO: '||PAGOS(PAGOS.LAST).PAGO||' Y SE DEBE DEVOLVER: '||DEVOLUCION);      
    END IF;
  END;
END;
...
TIPO_CLIENTE (1, 1049645321, 'Pedro Hernandez',3132567840,'C','M','Piel sensible'),
PAYMENTS_LIST(TYPE_PAYMENT (1, TO_TIMESTAMP('2017/06/04 10:13:18', 'YYYY/MM/DD HH:MI:SS'), 50000)),
APPOINMENTS_LIST(TYPE_APPOINMENT (1,TO_TIMESTAMP('2017/06/04 10:13:18', 'YYYY/MM/DD HH:MI:SS'),TO_TIMESTAMP('2017/07/04 10:13:18', 'YYYY/MM/DD HH:MI:SS'))),
TIPO_TATOOARTIST (1, 1069635221, 'Miguel Angel', 3112467862, 'C', 'M', 'PELAO', 'PELAO@GMAIL.COM',  OBJECT_CONTRACT(1, 'F', 'contrato a termino fijo',TO_DATE('2017/01/01','yyyy/mm/dd'), TO_DATE('2020-01-01','yyyy/mm/dd')), 'C://TATUADORES//PELAO.PNG', 'CAMBIAR POR LA LISTA DE TRABAJOS'),
TYPE_TATOO (2, 'Dragon', 'Dragon estilo oriental','C://tatoos//dragon.png'),
200000
));
INSERT INTO TATOOWORKS VALUES(TYPE_TATOOWORK (3,
TIPO_CLIENTE (2, 1149312311, 'Carlos Andres',3132567840,'C','M','Piel sensible'),
PAYMENTS_LIST(TYPE_PAYMENT (1, TO_TIMESTAMP('2017/06/04 10:13:18', 'YYYY/MM/DD HH:MI:SS'), 50000),
              TYPE_PAYMENT (2, TO_TIMESTAMP('2017/06/05 10:13:18', 'YYYY/MM/DD HH:MI:SS'), 60000)),
APPOINMENTS_LIST(TYPE_APPOINMENT (1,TO_TIMESTAMP('2017/06/04 10:13:18', 'YYYY/MM/DD HH:MI:SS'),TO_TIMESTAMP('2017/07/04 10:13:18', 'YYYY/MM/DD HH:MI:SS'))),
TIPO_TATOOARTIST (1, 1069635221, 'Miguel Angel', 3112467862, 'C', 'M', 'PELAO', 'PELAO@GMAIL.COM',  OBJECT_CONTRACT(1, 'F', 'contrato a termino fijo',TO_DATE('2017/01/01','yyyy/mm/dd'), TO_DATE('2020-01-01','yyyy/mm/dd')), 'C://TATUADORES//PELAO.PNG', 'CAMBIAR POR LA LISTA DE TRABAJOS'),
TYPE_TATOO (2, 'Dragon', 'Dragon estilo oriental','C://tatoos//dragon.png'),
200000
...
--CONSULTA
SELECT T.COSTO_TOTAL,T.PAGO(),T.DEUDA() FROM TATOOWORKS T;
